...
{{--
    ...
    - Theme system with localStorage + DB sync
...
>
    <head>
        ...
        {{-- User theme preference for server-side reference --}}
        <meta name="user-theme" content="{{ auth()->user()?->theme_preference ?? config('themes.default', 'corporate') }}" />
        ...
        <script>
            (function() {
                ...
                const savedTheme = localStorage.getItem(storageKey) || '{{ auth()->user()?->theme_preference ?? '' }}' || defaultTheme;
